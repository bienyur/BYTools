...
//    [self addSubview:self.pageControl]; 隐藏pagecontrol //////////
    ...
    self.skipButton = [[UIButton alloc] initWithFrame:CGRectMake(110*autoSizeScaleX, self.pageControl.frame.origin.y - margin*autoSizeScaleY, 100*autoSizeScaleX, 30*autoSizeScaleY)];
    
    [self.skipButton setAutoresizingMask: UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleRightMargin];
    [self.skipButton setBackgroundColor:[UIColor colorWithRed:47.0/255.0 green:143.0/255.0 blue:226.0/255.0 alpha:1.0f]];
    [self.skipButton.layer setMasksToBounds:YES];
    [self.skipButton.layer setCornerRadius:3.0f];
    [self.skipButton setTitle:GetLocalResStr(@"mx_immediate_use") forState:UIControlStateNormal];
...
- (void) setCurrentPage:(NSInteger)secondPage {
    if ([[[UIDevice currentDevice]systemVersion]floatValue] <7.0)
    {
        for (NSUInteger subviewIndex = 0; subviewIndex < [self.pageControl.subviews count]; subviewIndex++) {
            ...
            {
                [subview setImage:[UIImage imageNamed:@"mx_dot_blue"]];
            }
            else
            {
                [subview setImage:[UIImage imageNamed:@"mx_dot_gray"]];
            }
        }
    }
    else
    {
        ...
        {
            UIView* dotView = [self.pageControl.subviews objectAtIndex:i];
            UIImageView* dot = nil;
            ...
            if (dot == nil)
            {
                dot = [[[UIImageView alloc] initWithFrame:CGRectMake(0.0f, 0.0f, 10, 10)] autorelease];
                [dotView addSubview:dot];
            }
            if (i == secondPage)
            {
                dot.image = [UIImage imageNamed:@"mx_dot_blue"];
            }
            else
            {
                dot.image = [UIImage imageNamed:@"mx_dot_gray"];
            }
        }
    }
...
- (void)showPanelAtPageControl {
    ...
    [self.scrollView setContentOffset:CGPointMake(self.currentPageIndex * appWidth, 0) animated:YES];
}
...
- (void)relayoutSubviews:(CGSize)size{
    __block CGFloat xIndex=0;
    self.scrollView.frame=self.frame;
    [pageViews enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        UIView *tempView=(UIView *)obj;
        tempView.frame=CGRectMake(xIndex, 0, self.scrollView.frame.size.width, self.scrollView.frame.size.height);
        for (UIView *tempV in tempView.subviews) {
            if ([tempV isKindOfClass:[UIImageView class]]) {
                ((UIImageView *)tempV).image=((EAIntroPage *)_pages[idx]).titleImage;
                CGRect rect1 = tempView.frame;
                rect1.origin.x = (self.scrollView.frame.size.width - rect1.size.width)/2;
                rect1.origin.y = ((EAIntroPage *)_pages[idx]).imgPositionY;
                rect1.size=((EAIntroPage *)_pages[idx]).titleImage.size;
                tempV.frame=rect1;
            }
        }
        xIndex += self.scrollView.frame.size.width;
    }];
    self.scrollView.contentSize = CGSizeMake(xIndex, self.scrollView.frame.size.height);
    int margin = 45;
    if(appHeight>480)
    {
        margin = 55;
    ...
    self.skipButton.frame=CGRectMake(110*autoSizeScaleX, self.pageControl.frame.origin.y - margin*autoSizeScaleY, 100*autoSizeScaleX, 30*autoSizeScaleY);
}
@end
